...
class hb_frameless_OT_create_upper_bottom_detail(bpy.types.Operator):
    ...
    def _draw_cabinet_side_detail(self, context, scene, props):
        ...
        wall_cabinet_location = props.default_wall_cabinet_location
        ...
        # Draw cabinet side profile - bottom corner section of upper
        ...
        # Go back along bottom edge (4" toward back)
        ...
        # --- FLOOR LINE ---
        # Floor is at -wall_cabinet_location from bottom of upper
        floor_y = -wall_cabinet_location
        ...
        floor_line.set_point(0, Vector((detail_left, floor_y, 0)))
        floor_line.add_point(Vector((detail_right, floor_y, 0)))
        ...
        floor_text.set_location(Vector((detail_right + units.inch(0.25), floor_y, 0)))
        floor_text.set_alignment('LEFT', 'CENTER')
        
        # --- WALL CABINET LOCATION DIMENSION ---
        # Dimension from floor to bottom of upper cabinet
        wcl_dim = hb_types.GeoNodeDimension()
        wcl_dim.create("Wall Cabinet Location Dimension")
        wcl_dim.obj.location = Vector((-corner_size - units.inch(0.5), floor_y, 0))
        wcl_dim.obj.rotation_euler.z = math.pi / 2
        wcl_dim.obj.data.splines[0].points[1].co = (wall_cabinet_location, 0, 0, 1)
        wcl_dim.set_input("Leader Length", units.inch(-0.5))
        wcl_dim.set_decimal()
        ...
        text.set_location(Vector((0, floor_y - units.inch(1), 0)))
